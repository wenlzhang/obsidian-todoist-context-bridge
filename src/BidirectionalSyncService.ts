...
export class BidirectionalSyncService {
    ...
    > {
        ...
        // Journal-based sync optimization is used instead of time window filtering
        ...
        for (const file of files) {
            try {
                ...
                // Process all files - journal-based optimization handles efficiency
        ...
        try {
            for (const todoistId of uniqueTodoistIds) {
                try {
                    ...
                    if (task) {
                        // All linked tasks are processed - journal-based optimization handles efficiency
                        {
                            ...
                            const v2Id = await this.todoistV2IDs.getV2Id(
                                task.id,
                            );
                            if (v2Id !== task.id) {
