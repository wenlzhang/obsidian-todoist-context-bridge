...
export class TodoistToObsidianModal extends Modal {
    ...
    onOpen() {
        ...
        const linkInput = inputContainer.createEl("input", {
            ...
            placeholder:
                "https://app.todoist.com/app/task/example-task-id or example-task-id",
        });
            ...
            try {
                ...
                for (const currentId of idParts) {
                    ...
                    try {
                        ...
                        if (
                            /^\d+$/.test(currentId) ||
                            /^[a-zA-Z0-9]{16}$/.test(currentId)
                        ) {
                            try {
                                const task =
                                    await this.plugin.todoistApi.getTask(
                                        currentId,
                                    );
                                if (task) {
                                    matchedTask = task;
                    ...
                    if (!matchedTask && allTasks.length > 0) {
                        // Try direct ID match
                        matchedTask =
                            allTasks.find((task) => task.id === currentId) ||
                            null;
                        if (matchedTask) {
                            break;
                        }
                        ...
                        for (const task of allTasks) {
                            ...
                            if (
                                /^[a-zA-Z0-9]{16}$/.test(currentId) &&
                                taskUrl.endsWith(currentIdLower)
                            ) {
                                matchedTask = task;
                                break;
                            }
